...
def participating_nations_over_time(df):
    ...
    nations_over_time.sort_values(by=list[0], inplace=True, axis=1)
    nations_over_time.rename(
        columns={"index": "Edition", "Year": "No of Countries"}, inplace=True
    )
...
def most_sucssesful_athletes_countrywise(df, region):
    temp_df = df.dropna(subset=["Medal"])
